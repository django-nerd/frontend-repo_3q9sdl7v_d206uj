...
const testimonials = [
  {
    quote: "iLearn cut our onboarding time in half. New hires ship customer-ready messages after a week, not a month.",
  ...
  return (
    <section className="py-20 bg-[#F8F8FF]">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-[#2F2E41]">What iLearn users say</h2>
        <div className="mt-10 relative">
          <div className="overflow-hidden">
            <div className="flex transition-transform duration-700 ease-out" style={{ transform: `translateX(-${index * 100}%)` }}>
              {testimonials.map((t, i) => (
                <div key={i} className="min-w-full px-4">
                  <div className="rounded-2xl border border-gray-100 bg-white/80 backdrop-blur shadow-md p-8">
                    <p className="text-lg md:text-xl text-[#2F2E41] leading-relaxed">“{t.quote}”</p>
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            {testimonials.map((_, i) => (
              <button key={i} aria-label={`Go to slide ${i+1}`} onClick={() => setIndex(i)} className={`h-2.5 w-2.5 rounded-full transition-colors ${i === index ? 'bg-[#2F2E41]' : 'bg-slate-300 hover:bg-slate-400'}`} />
            ))}
          </div>
        </div>
      </div>
